    ...
    <div style="background:var(--bg-secondary);border-radius:10px;padding:12px 14px;margin-bottom:8px;border:1px solid var(--border);${addr.isPrimary ? 'border-left:3px solid #1e3a5f;' : ''}">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;">
        ...
        <div style="display:flex;gap:4px;margin-left:8px;">
          <button onclick="openProvidersModal('${addr.id}', '${(addr.label || '').replace(/'/g, "\\'")}')" style="background:none;border:1px solid #e5e7eb;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;" title="Prestataires">🔧</button>
...
// ─── Known People & Planned Interventions ──────────────────────────────
...
function renderKnownPeople() {
  ...
  if (providersKnownPeople.length === 0) {
    container.innerHTML = '<p style="color:#9ca3af;font-size:12px;padding:4px 0;">Aucune personne enregistrée</p>';
  } else {
    container.innerHTML = providersKnownPeople.map(p => `
      <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:10px 12px;margin-bottom:6px;">
        <div>
          <div style="font-weight:700;font-size:13px;color:#1f2937;">${p.name} <span style="font-weight:500;color:#6b7280;font-size:12px;">${PROVIDER_CATEGORY_LABEL[p.category] || p.category}</span></div>
          ${p.company ? `<div style="font-size:12px;color:#6b7280;">${p.company}</div>` : ''}
          ${p.phone ? `<div style="font-size:12px;color:#6b7280;">📞 ${p.phone}</div>` : ''}
          ${p.vehiclePlate ? `<div style="font-size:12px;color:#6b7280;">🚗 ${p.vehiclePlate}</div>` : ''}
          ${p.notes ? `<div style="font-size:12px;color:#9ca3af;">${p.notes}</div>` : ''}
        </div>
        <button onclick="deleteKnownPerson('${p.id}')" style="background:none;border:1px solid #fecaca;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;color:#dc2626;">🗑️</button>
      </div>`).join('');
  }
  const select = document.getElementById('interventionPersonId');
  select.innerHTML = providersKnownPeople.map(p => `<option value="${p.id}">${p.name}</option>`).join('') || '<option value="">Aucun prestataire — ajoutez-en un d\'abord</option>';
}
...
function renderInterventions() {
  ...
  if (providersInterventions.length === 0) {
    container.innerHTML = '<p style="color:#9ca3af;font-size:12px;padding:4px 0;">Aucune intervention prévue</p>';
    return;
  }
  container.innerHTML = providersInterventions.map(iv => `
    <div style="display:flex;justify-content:space-between;align-items:flex-start;background:#fff;border:1px solid #e5e7eb;border-radius:10px;padding:10px 12px;margin-bottom:6px;">
      <div>
        <div style="font-weight:700;font-size:13px;color:#1f2937;">${iv.personName}</div>
        <div style="font-size:12px;color:#6b7280;">${formatInterventionDate(iv.scheduledStart, iv.recurrence)}</div>
        ${iv.notes ? `<div style="font-size:12px;color:#9ca3af;">${iv.notes}</div>` : ''}
      </div>
      <button onclick="deleteIntervention('${iv.id}')" style="background:none;border:1px solid #fecaca;border-radius:6px;padding:4px 8px;cursor:pointer;font-size:11px;color:#dc2626;">🗑️</button>
    </div>`).join('');
}
...
async function deleteKnownPerson(personId) {
  ...
  try {
    ...
    showToast('🗑️ Personne retirée', 'success');
    await loadProviders();
  } catch (e) {
    showToast('❌ Erreur', 'error');
  }
...
async function deleteIntervention(interventionId) {
  ...
  try {
    ...
    showToast('🗑️ Intervention annulée', 'success');
    await loadProviders();
  } catch (e) {
    showToast('❌ Erreur', 'error');
  }
